...
import pylab as plt
import os
def get_poseimg(all_peaks, save_path = None, img_path = None):
...
def main():
	...
	parser.add_argument('--json_path', type=str, default='./dataset/pose')
	...
	for file in tqdm.tqdm(all_files):
		if "cloth" in file:
			continue
		keypoints = json.load(open(file))["people"][0]["pose_keypoints_2d"] #changed pose_keypoints to pose_keypoints_2d
		keypoints = np.resize(keypoints,(18,3))
		save_path_ = os.path.join(save_path,file.split('/')[-1].replace('json','jpg'))
		get_poseimg(keypoints, save_path_)
