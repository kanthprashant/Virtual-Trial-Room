## https://github.com/OFA-Sys/DAFlow
...
def get_img_agnostic(img, parse, pose_data):
    ...
    # print(f"parse_head: {parse_head.shape}, parse_lower: {parse_lower.shape}, parse_hand: {parse_hand.shape}")
...
def process(img_name, data_path, load_width, save_dirname="img_agnostic"):
    parse_name = img_name.replace('.jpg', '.png')
    parse = Image.open(osp.join(data_path, 'image-parse-new', parse_name)).convert('L')  ###### added .convert('L')
    ...
    with open(osp.join(data_path, 'pose', pose_name), 'r') as f:
        ...
        ###### if keypoints doesn't exist remove data [PK]
        if len(pose_label['people']) == 0: 
            os.remove(osp.join(data_path, 'image-parse-new', parse_name))
            os.remove(osp.join(data_path, 'lip_seg', parse_name))
            os.remove(osp.join(data_path, 'pose', pose_name))
            os.remove(osp.join(data_path, 'image', img_name))
            print(f"removed: {img_name}")
            return
        ###### if keypoints doesn't exist remove data [PK]
        pose_data = pose_label['people'][0]['pose_keypoints_2d'] #changes pose_keypoints to pose_keypoints_2d
...
def process_onlypose(img_name, data_path, load_width):
    parse_name = img_name.replace('.jpg', '.png')
    parse = Image.open(osp.join(data_path, 'image-parse-new', parse_name)).convert('L')  ###### added .convert('L')
    parse = transforms.Resize(load_width, interpolation=0)(parse)
    pose_name = img_name.replace('.jpg', '_keypoints.json')
...
def main(opt):
    ...
    with open(opt.data_txt, 'r') as f:
        for line in tqdm.tqdm(f.readlines()):
            ...
            if opt.onlypose:
                process_onlypose(img_name, data_path, load_width)
            else:
                process(img_name, data_path, load_width)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_path', type=str, default='')
    parser.add_argument('--data_txt',  type=str, default='')
